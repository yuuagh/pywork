...
def lineMethod(fileName):
    ...
    for line in lines:
        content = re.match(r'\s{5}CDS\s+(\S+)\n', line, re.S | re.I)
...
def getAllData2(dirname, fname, name):
    ...
    for unit in dic:
        ...
        testContent = '>name = ' + name + ', protein_id = ' + unit + ', ' + 'gene = ' + geneDic[unit] + ', position = ' + pt + ":\n"
        fl.write(testContent)
        #print testContent
        ...
        for k in dic[unit]:
            ...
            fl.write(final)
            print 'start = ' + str(value[0]) + ', end = ' + str(value[1]) + ', 参考长度 = ' + str(int(value[1]) - int(value[0]) + 1) + ', 截取长度 = ' + str(len(final))
            if int(value[1]) - int(value[0]) + 1 != len(final):
                print '提醒： 截取长度不一致,' + ', 参考长度 = ' + str(int(value[1]) - int(value[0]) + 1) + ', 截取长度 = ' + str(len(final))
