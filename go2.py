...
def getSingleData(dirname, fname):
    dic         = getTargetData(fname)
    content     = getContent(fname)
    inpath      = dirname + '\\simple'
    length      = len(content)
    for unit in dic:
        fl = open(dirname + '\\simple\\'+ unit, 'wb')
        for k in dic[unit]:
            fl.write(k + ":\n")
...
def getAllData(dirname, fname):
    dic         = getTargetData(fname)
    content     = getContent(fname)
    inpath      = dirname + '\\all'
    fl          = open(dirname + '\\all\\all.txt', 'wb')
    length      = len(content)
    for unit in dic:
        for k in dic[unit]:
            fl.write(k + ":\n")
...
def getAllData2(dirname, fname, name):
    ...
    for unit in dic:
        for k in dic[unit]:
            fl.write('name = ' + name + ', protein_id = ' + unit + ', ' + 'gene = ' + geneDic[unit] + ', position = ' + k  + ":\n")
            # 字符串 content 是从0的下标开始
            value   = re.split(r'\.\.', k)
            bg      = int(value[0]) - 1 
            ed      = int(value[1]) - 1
...
def getDirData():
    ...
    for name in names:
        try:
            getSingleData(curfile + '\\result' + '\\' + name, curfile + '\\source\\' + name)
            getAllData(curfile + '\\result' + '\\' + name, curfile + '\\source\\' + name)
